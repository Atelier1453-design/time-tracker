...
**複数のマシンから作業されている**：作者は職場PCと自宅PCの両方から、別々の Claude Code
セッションでこのリポジトリを触っている。過去に一度、両方から並行してpushしようとして
コンフリクトになったことがある（2026-09-02）。作業を始める前に必ず `git fetch origin` と
`git log HEAD..origin/main --oneline` で相手側の新しいコミットがないか確認し、あればpush前に
`git merge origin/main` で取り込むこと。
...
`np`（行動名のあとの助詞）・`startWords`/`endWords`/`overlapWords` はどれも「＋追加」で増やせる
共有リスト（`particles`/`startWords`/`endWords`/`overlapWords` として保存）。

**日記の並び順**：行動ごとではなく「文ごと」の実際の時刻で並べる（`composeDiary`内の`timed`配列）。
行動ごとの「その日はじめての時刻」で並べていた頃は、前日から続く記録がその日の0:00にクリップされる
ことで行動全体が不自然に先頭へ来てしまうバグがあった（2026-09-02修正）。また、同じホスト・同じ
`overlap`の従属文が並び順で連続する場合は、ホストの文言を繰り返さず1文にまとめる
...
- カラーバーを指でなぞると出る欄（プローブパネル）から、記録の**追加・編集・削除**がすべてできる。
  かつて存在した「時間を修正」タブは、この機能に置き換わったため削除済み。追加時は「計測中のまま
  追加」も選べる（終了時刻を決めずに追加できる）
- 計測開始から3秒未満で止めた記録は自動で破棄（誤タップ対策）、
  止めてから8秒だけ「元に戻す」を出す
- 設定タブは「日記全体の書き方／行動ボタン／保存とデータ」のアコーディオン（初期状態は全部閉じる）
- 日記が、生成後に記録や設定が変わって古くなっていたら警告と「作り直す」を出す（`diaryStale`）
- カラーバー下に「前の区切り／次の区切り」ボタンで、記録の始まり・終わり目に縦線をジャンプできる
- 行動編集中に貼りつく「できあがる文」プレビューは `top: max(76px, env(safe-area-inset-top) + 24px)`
  で、iPhoneのステータスバー等と重ならない位置に固定
- 行動の色は用意した色に加えて `<input type="color">` で自由に選べる
...
`normalize()` を触るときは注意すること。`base`オブジェクトに`overlap`を含めていないのは意図的
（含めると「未設定」判定ができず、旧設定からの引き継ぎが効かなくなる）。
